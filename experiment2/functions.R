...
scoreQuestionnaires <- function(bfi10) {
  # # # # # # # # # # # # # # # SCL90-R # # # # # # # # # # # # # # #
  # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
  ...
  colnames(bfi10_total) <- paste0("bfi_",subscales)
  ...
  # return
  return(list(bfi=bfi10_total))
}


# add questionnaires to ratings
addQuestionnaireToRating <- function (ratings,bfi10) {
  # create an empty matrix
  temp <- matrix(NA,nrow=nrow(ratings),ncol=ncol(bfi10)-1)
  colnames(temp) <- c(colnames(bfi10)[-1])
  # add temp to ratings
  ratings <- cbind(ratings,temp)
  
  # combine questionnaires
  quests <- cbind(bfi10[-1])
  ...
  for (i in 1:length(good_ids)) {
    ...
    if (i == 1) {
      output <- data.frame(temp1, likert=temp2$Response,
                           temp2[,grepl("q_bfi",colnames(temp2))])
    } else {
